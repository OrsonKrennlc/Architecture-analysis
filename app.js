...
const mkPin=(dark)=>L.divIcon({className:'',iconSize:[12,12],iconAnchor:[6,6],
...
async function doSearch(){
  ...
  try{
    ...
    if(curView==='3d'&&window.S3&&S3.flyTo)S3.flyTo(sLat,sLng);
  }catch(e){
...
function drawRoads(data,sc){
  ...
  const RD={motorway:{c:s.p,w:2.4,glow:true},trunk:{c:s.p,w:2.1,glow:true},
...
function drawContext(sc){
  ...
  const rs=SCH.roads[0];
  const RD={motorway:{c:rs.p,w:2.2},trunk:{c:rs.p,w:2},primary:{c:rs.p,w:1.8},
...
function drawNoise(data,sc){
  const s=SCH.noise[sc||0];
  const b=getBounds();
  const fid='ns'+Math.random().toString(36).slice(2,8);
  const NL={motorway:{db:75,w:18},trunk:{db:73,w:16},primary:{db:70,w:13},
...
function drawPoiHeat(data,sc){
  const s=SCH.poiheat[sc]||SCH.poiheat[0];
  const b=getBounds();
...
function drawPoiType(data,sc){
  const s=SCH.poitype[sc]||SCH.poitype[0];
  const b=getBounds();
  function cat(t){
...
function drawCombined(sc){
  const s=SCH.combined[sc]||SCH.combined[0];
  const b=getBounds();
  let inner='<rect width="'+W+'" height="'+H+'" fill="'+s.bg+'"/>';
  // green areas
  (lastD.gw&&lastD.gw.elements||[]).forEach(el=>{
...
function drawAccess(data,sc){
  ...
  const hwSpeed={motorway:1.0,trunk:1.0,primary:1.2,secondary:1.2,
...
const I18N={
  ...
  accent_color:{en:'ACCENT COLOR · 主色',zh:'ACCENT COLOR · 自定义主色'},
...
async function generate(){
  ...
  const needNodes=ids.has('nodes')||ids.has('vitality')||ids.has('poiheat')||ids.has('poitype');
  ...
  // If nothing came back, OSM is busy/unreachable — show friendly message and stop
  if(allEls.length===0){
    unload();
    document.querySelector('.gen').classList.add('ready');
    showBusyMsg();
  ...
  D.roads={elements:allEls.filter(e=>{
    const hw=e.tags&&e.tags.highway;
    return hw&&hwRe.test(hw);
  })};
  D.landuse={elements:allEls.filter(e=>{
    const t=e.tags||{};
  ...
  D.buildings={elements:allEls.filter(e=>{
    const t=e.tags||{};
    return e.geometry&&t.building;
  })};
  D.context={elements:allEls.filter(e=>{
    const t=e.tags||{};
    return t.building; // all buildings including large radius
  })};
  D.transit={elements:allEls.filter(e=>{
    const t=e.tags||{};
  ...
  D.pois={elements:allEls.filter(e=>e.type==='node'&&e.tags&&
  ...
  grid.innerHTML=sel.map((a,i)=>{
    ...
    const prev=PREV[a.id]||[['#AAA','#888'],['#777','#666'],['#555','#444']];
    const dots=prev.map((pair,si)=>'<div class="dot'+(si===0?' on':'')+'" style="background:linear-gradient(135deg,'+pair[0]+' 50%,'+pair[1]+' 50%)" onclick="schSwitch(\''+a.id+'\','+si+',event)" title="Scheme '+(si+1)+'"></div>').join('');
    const defName = lang === 'zh' ? (a.name_zh || a.name) : a.name;
...
/* ════════════════════════════════════════════════════════════════
   3D CITY MODULE — namespaced to avoid collisions with 2D app
════════════════════════════════════════════════════════════════ */
const S3=(function(){
  let map3=null, BL=null, BSRC=null, BSL=null, curMode='white', shape='circle';
  ...
  const MODES=[
    ...
    {m:'zone',name:'Land Use Zoning',tag:'ZONE',leg:[{c:'#F4A261',v:'Residential'},{c:'#E9C46A',v:'Mixed'},{c:'#E76F51',v:'Commercial'},{c:'#457B9D',v:'Office'},{c:'#1D3557',v:'Landmark'}]},
    {m:'density',name:'Building Density',tag:'DENS',leg:[{c:'#FFFFCC',v:'Very low'},{c:'#7FCDBB',v:'Mid-low'},{c:'#41B6C4',v:'Medium'},{c:'#225EA8',v:'High'},{c:'#0C2C84',v:'Very high'}]},
    {m:'solar',name:'Solar Study',tag:'SUN',leg:[{c:'#FFE08A',v:'Sunlit'},{c:'#6E5B4E',v:'Shaded'},{c:'#C0392B',v:'Sun dir →'}]},
    {m:'skyline',name:'Skyline Profile',tag:'SECT',leg:[{c:'#1A1917',v:'Silhouette'},{c:'#C0392B',v:'Peak'},{c:'#A8A69F',v:'Ruler'}]},
  ];

  function ensureMap(){
    if(inited)return;
    inited=true;
    const clat=sLat||48.8566, clng=sLng||2.3522;
    map3=new maplibregl.Map({
      container:'map3d',
      style:'https://tiles.openfreemap.org/styles/liberty',
      center:[clng,clat],zoom:15.5,pitch:55,bearing:45,
      antialias:true,maxPitch:80,preserveDrawingBuffer:true,
    });
    map3.addControl(new maplibregl.NavigationControl(),'top-right');
    map3.on('load',()=>{
      map3.getStyle().layers.forEach(l=>{if(l.type==='symbol'){try{map3.setLayoutProperty(l.id,'visibility','none');}catch(e){}}});
      const bl=map3.getStyle().layers.filter(l=>l['source-layer']==='building'||/building/i.test(l.id));
      if(bl.length){BSRC=bl[0].source;BSL=bl[0]['source-layer']||'building';bl.forEach(l=>{try{map3.setLayoutProperty(l.id,'visibility','none');}catch(e){}});}
      else{BSRC='openmaptiles';BSL='building';}
      BL='s3_bldg';
      try{
        map3.addLayer({id:BL,type:'fill-extrusion',source:BSRC,'source-layer':BSL,minzoom:12,
          paint:{'fill-extrusion-color':'#e8e2d4',
            'fill-extrusion-height':['interpolate',['linear'],['zoom'],12,0,13,['coalesce',['get','render_height'],['get','height'],8]],
            'fill-extrusion-base':['interpolate',['linear'],['zoom'],12,0,13,['coalesce',['get','render_min_height'],['get','min_height'],0]],
            'fill-extrusion-opacity':1}});
        map3.setLight({anchor:'map',color:'#fff5e0',intensity:0.5,position:[1.5,210,45]});
        SM('white');
      }catch(e){console.error('3D layer err',e);}
    });
  }

  function flyTo(lat,lng){
    if(!map3)return;
    map3.flyTo({center:[lng,lat],zoom:15.5,pitch:55,bearing:45,duration:1200});
  }

  const H=()=>['coalesce',['get','render_height'],['get','height'],3];
  function setColor(e){if(map3&&map3.getLayer(BL))try{map3.setPaintProperty(BL,'fill-extrusion-color',e);}catch(x){}}
  function setOpacity(v){if(map3&&map3.getLayer(BL))try{map3.setPaintProperty(BL,'fill-extrusion-opacity',v);}catch(x){}}
  ...
  function showLeg(m){
    ...
    if(md)document.getElementById('leg3d').innerHTML=legHTML(md.leg);
  }

  function SM(m){
    curMode=m;
    ['white','height','zone','density','solar','skyline'].forEach(k=>{
      const b=document.getElementById('m3d_'+k);if(b)b.classList.toggle('on',k===m);
    });
    document.getElementById('solar3d').style.display=m==='solar'?'block':'none';
    if(!map3||!map3.getLayer(BL))return;
    if(m==='white'||m==='skyline'){setColor('#e8e2d4');setOpacity(1);setLt('#fff5e0',.5,210,45);}
    else if(m==='height'){setColor(['step',H(),'#EDF8E9',6,'#BAE4BC',12,'#74C476',25,'#31A354',40,'#FFEDA0',60,'#FEB24C',80,'#FC4E2A',120,'#BD0026']);setOpacity(.97);setLt('#fff',.3,200,65);}
    else if(m==='zone'){setColor(['step',H(),'#A8DADC',4,'#F4A261',12,'#E9C46A',25,'#E76F51',50,'#457B9D',80,'#1D3557']);setOpacity(.97);setLt('#fff',.28,200,60);}
    else if(m==='density'){setColor(['step',H(),'#FFFFCC',5,'#C7E9B4',10,'#7FCDBB',20,'#41B6C4',35,'#1D91C0',55,'#225EA8',80,'#0C2C84']);setOpacity(.97);setLt('#fff',.28,200,60);}
    else if(m==='solar'){updateSolar();}
    showLeg(m);
  }

  function updateSolar(){
    const min=+document.getElementById('ssl3d').value,h=min/60|0,m=min%60;
    document.getElementById('sclock3d').textContent=String(h).padStart(2,'0')+':'+String(m).padStart(2,'0');
    const ps=[[0,'NIGHT'],[360,'DAWN'],[420,'SUNRISE'],[480,'MORNING'],[720,'SOLAR NOON'],[840,'AFTERNOON'],[1080,'SUNSET'],[1140,'DUSK'],[1200,'NIGHT']];
    let ph='NIGHT';for(const[t,n]of ps)if(min>=t)ph=n;document.getElementById('sph3d').textContent=ph;
    if(!map3||!map3.getLayer(BL))return;
    const lat=sLat||48.8566,lng=sLng||2.3522;
    const d=new Date(new Date().toISOString().slice(0,10)+'T'+String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')+':00');
    const pos=SunCalc.getPosition(d,lat,lng);
    const el=pos.altitude*180/Math.PI, az=((pos.azimuth*180/Math.PI)+180)%360;
    solarAz=az;solarAlt=el;
    if(el<=-8){setColor('#1C2833');setOpacity(.9);setLt('#0D1B2A',.05,az,85);}
    else if(el<0){const u=(el+8)/8;setColor('rgb('+Math.round(40+100*u)+','+Math.round(50+80*u)+','+Math.round(60+80*u)+')');setOpacity(.93);setLt('rgb(255,'+Math.round(80+100*u)+',20)',.06+u*.15,az,2);}
    else{const t=Math.min(el/65,1);const top='rgb('+Math.round(230+20*t)+','+Math.round(205+30*t)+','+Math.round(160+60*t)+')';const bot='rgb('+Math.round(100+80*t)+','+Math.round(110+75*t)+','+Math.round(120+65*t)+')';setColor(['step',H(),bot,18,bot,19,top,200,top]);setOpacity(.97);setLt('rgb(255,'+Math.round(150+105*t)+','+Math.round(40+215*t)+')',0.1+t*.78,az,Math.max(el,.5));}
  }
  document.getElementById('ssl3d').addEventListener('input',updateSolar);

  function setShape(s){
    shape=s;
    document.getElementById('shp3d_circle').classList.toggle('on',s==='circle');
    document.getElementById('shp3d_square').classList.toggle('on',s==='square');
  }

  function waitIdle(){return new Promise(res=>{let d=false;const f=()=>{if(!d){d=true;res();}};map3.once('idle',f);setTimeout(f,1400);});}

  function maskPath(ctx,OUT){
    ctx.beginPath();
    if(shape==='circle'){ctx.arc(OUT/2,OUT/2,OUT/2-4,0,Math.PI*2);}
    else{const r=40,p=4,w=OUT-p*2;ctx.moveTo(p+r,p);ctx.arcTo(p+w,p,p+w,p+w,r);ctx.arcTo(p+w,p+w,p,p+w,r);ctx.arcTo(p,p+w,p,p,r);ctx.arcTo(p,p,p+w,p,r);}
    ctx.closePath();
  }
  function captureCard(mode){
    const src=map3.getCanvas();
    const sw=src.width,sh=src.height,side=Math.min(sw,sh)*0.86,sx=(sw-side)/2,sy=(sh-side)/2,OUT=600;
    const c=document.createElement('canvas');c.width=OUT;c.height=OUT;
    const ctx=c.getContext('2d');
    ctx.fillStyle='#F5F4F2';ctx.fillRect(0,0,OUT,OUT);
    ctx.save();maskPath(ctx,OUT);ctx.clip();
    ctx.drawImage(src,sx,sy,side,side,0,0,OUT,OUT);
    if(mode==='solar'&&solarAz!=null)drawSunArrow(ctx,OUT,solarAz,solarAlt);
    ctx.restore();
    ctx.strokeStyle='#D8D6D0';ctx.lineWidth=1;ctx.setLineDash([4,4]);maskPath(ctx,OUT);ctx.stroke();ctx.setLineDash([]);
    return c.toDataURL('image/png');
  }
  function drawSunArrow(ctx,OUT,az,alt){
    const cx=OUT/2,cy=OUT/2,ang=(az-45-90)*Math.PI/180,r1=OUT*0.16,r2=OUT*0.40;
    const x1=cx+Math.cos(ang)*r1,y1=cy+Math.sin(ang)*r1,x2=cx+Math.cos(ang)*r2,y2=cy+Math.sin(ang)*r2;
    ctx.strokeStyle='rgba(192,57,43,0.25)';ctx.lineWidth=10;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke();
  ...
  function buildSkyline(){
    const OUT=600,c=document.createElement('canvas');c.width=OUT;c.height=OUT;const ctx=c.getContext('2d');
    ctx.fillStyle='#F7F6F3';ctx.fillRect(0,0,OUT,OUT);ctx.save();maskPath(ctx,OUT);ctx.clip();ctx.fillStyle='#F7F6F3';ctx.fillRect(0,0,OUT,OUT);
    let feats=[];try{feats=map3.queryRenderedFeatures({layers:[BL]});}catch(e){}
  ...
  async function generateSet(){
    if(!map3||!map3.getLayer(BL)){toast('3D MAP NOT READY');return;}
    load('GENERATING 3D ANALYSIS...');
    const saved=curMode;
    map3.easeTo({pitch:55,bearing:45,duration:500});
    await new Promise(r=>setTimeout(r,650));
    cards=[];
    for(const am of MODES){
      let img;
      if(am.m==='skyline'){SM('white');await waitIdle();img=buildSkyline();}
      else{SM(am.m);if(am.m==='solar'){document.getElementById('ssl3d').value=720;updateSolar();}await waitIdle();await new Promise(r=>setTimeout(r,250));img=captureCard(am.m);}
      cards.push({...am,img});
    }
    SM(saved);
    unload();
  ...
  function exportReportPDF(){
    if(!cards.length){toast('GENERATE FIRST');return;}
    const loc=(sName||'Site').slice(0,40);
    const date=new Date().toLocaleDateString('en-GB',{year:'numeric',month:'long',day:'numeric'});
    let rows='';
    cards.forEach((c,i)=>{
  ...
  function exportPoster(){
    ...
    const loc=(sName||'Site').slice(0,40);
    const date=new Date().toLocaleDateString('en-GB',{year:'numeric',month:'long',day:'numeric'});
    const cols=3,cardW=440,gap=26,pad=54,rows=Math.ceil(cards.length/cols),headH=120,cardH=cardW+92;
...
function setView(v){
  curView=v;
  document.getElementById('view2d').classList.toggle('on',v==='2d');
  document.getElementById('view3d').classList.toggle('on',v==='3d');
  const is3=v==='3d';
  document.getElementById('map3d').style.display=is3?'block':'none';
  document.getElementById('sec3d').style.display=is3?'block':'none';
  ['secOverlay','secTypes','secShape','secTheme','exp2d','exp2dP','exp2dL'].forEach(id=>{
    const el=document.getElementById(id);if(el)el.style.display=is3?'none':'';
  });
  const e3=document.getElementById('exp3d');if(e3)e3.style.display=is3?'':'none';
  const sb=document.getElementById('statBar');if(sb&&is3)sb.style.display='none';
  const gm=document.getElementById('genMain');
  if(gm)gm.innerHTML=is3?'GENERATE 3D ANALYSIS ↗':'GENERATE ANALYSIS ↗';
  const tl=document.getElementById('tileLight');
  if(tl)tl.style.display=is3?'none':'';
  if(is3){
    if(curStyle!=='sat'){const sb=document.getElementById('tileSat');if(sb)setTile(sb,'sat');}
    S3.ensureMap();
    setTimeout(()=>{
      if(S3.resize)S3.resize();
      if(sLat)S3.flyTo(sLat,sLng);
    },350);
  }
}
function generateMain(){
  if(curView==='3d')S3.generateSet();
  else generate();
}
...
const HELP_STEPS=[
  ...
  ['03',
   ...
   {en:'Tick the analysis types you want, then press GENERATE. Switch to 3D City for building massing.', zh:'勾选需要的分析类型，点击 GENERATE。切换 3D City 可做建筑体量分析。'}
  ],
  ['04',
...
function SM3(m){S3.SM(m);}
function setShape3(s){S3.setShape(s);}
function generateAnalysisSet3(){S3.generateSet();}
